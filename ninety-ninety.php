...
if ( ! class_exists( 'NinetyNinety' ) ):
	...
	class NinetyNinety {
		...
		//* Plugin settings
		public $settings = [];
		//* Plugin options, saved in options table
		public $options = [];
		...
		public function initialize() {
			...
			$this->settings['page_templates'] = apply_filters( 'ninety-page-templates', [
		...
		public function add_actions_and_filters() {
			...
			add_action( 'acf/save_post', [
				$this,
				'set_meeting_title_time',
			], PHP_INT_MAX ); //* Run as late as possible
			if ( ! class_exists( 'ACF' ) ) {
				add_filter( 'acf/settings/dir', [
					$this,
					'acf_settings_dir',
				] ); //* acf/settings/url is deprecated
			}
			...
			add_filter( 'acf/load_field/key=field_5d182c40c6e57', [
				$this,
				'set_default_meeting_time',
			] );
			add_filter( 'acf/load_field/key=field_5d197bc132ced', [
				$this,
				'ninety_coords_readonly',
			] );
			add_action( 'update_option_ninety_settings', [
				$this,
				'update_meeting_count'
			], 10, 2 );
			add_filter( 'acf/load_field/key=field_5d18480b686a6', [
				$this,
				'set_default_meeting_location',
			] );
			add_filter( 'acf/load_field/key=field_5d18255d071c8', [
				$this,
				'set_default_meeting_type',
			] );
			add_filter( 'acf/load_field/key=field_5d184b55fa43e', [
				$this,
				'filter_meeting_programs',
			] );
		}
		 ...
		 *
		 * @param $field array ACF button group field settings
		 *
		 * @return mixed
		 * @since 1.0.0
		 *
		...
		public function filter_meeting_programs( $field ) {
			...
			$field['choices'] = apply_filters( 'ninety-programs', $field['choices'] );
		...
		protected function get_meeting_data() {
			...
			$custom_marker_colors = apply_filters( 'ninety-meeting-colors', $default_marker_colors );
		 ...
		 *
		 * @param $field array ACF field for Meeting date/time
		 *
		 * @return mixed
		 * @since 1.0.0
		 *
		 */
		function set_default_meeting_time( $field ) {
		 ...
		 *
		 * @param $field array ACF field for Location coords
		 *
		 * @return mixed
		 * @since 1.1.0
		 *
		 */
		function ninety_coords_readonly( $field ) {
		...
		public function set_meeting_title_time( $post_id ) {
			...
			// Set Meeting title to date/time, update slug and update post meta with timestamp for ordering
		 ...
		 *
		 */
		function geocode_meeting_location( $term_id, $address ) {
		...
		/**
		 * Get lng/lat coords for address
		 *
		...
		protected function geo_lookup( $address ) {
			...
			if ( isset( $data[0]['center'] ) ) {
				$ret['center'] = $data[0]['center'];
			}
			
			if ( isset( $data[0]['place_name'] ) ) {
				$ret['address'] = $data[0]['place_name'];
			}
		...
		/**
		 * Updates Meeting count based on
		 *
		 * @return void
		 * @since 1.2.0
		 *
		 ...
		 *
		 * @param $item WP_POST nav_menu_item
		 *
		 * @return mixed
		 * @since 1.0.0
		 *
